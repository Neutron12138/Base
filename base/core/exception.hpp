...
namespace base
{
    ...
    {
        return std::invalid_argument(format_exception_message(debug_info, "(invalid_argument) ", std::forward<Ts>(args)...));
    }

    template <typename... Ts>
    inline std::logic_error make_out_of_range_error(const std::string &debug_info, Ts &&...args)
    {
        return std::out_of_range(format_exception_message(debug_info, "(out_of_range) ", std::forward<Ts>(args)...));
    }
