...
namespace base
{
    ...
    class GlobalField;
    template <typename T>
    using GlobalFieldRef = Ref<GlobalField<T>>;
    template <typename T>
    using GlobalFieldWeakRef = WeakRef<GlobalField<T>>;
    ...
    public:
        GlobalField(Ptr ptr) : FieldBase(false), m_ptr(ptr) {}
        GlobalField(ConstPtr ptr) : FieldBase(true), m_const_ptr(ptr) {}
